...
train_set = load_json(os.path.join(data_dir, 'IMCS-V2_train.json'))
dev_set = load_json(os.path.join(data_dir, 'IMCS-V2_dev.json'))
    ...
    for pid, sample in samples.items():
        for sent in sample['dialogue']:
            ...
            if 'dialogue_act' not in sent:
                out += x + '\t' + str(-1) + '\n' #对于test数据集，意图设为-1
           #assert sent['dialogue_act'] in tag2id, f"Invalid dialogue_act: {sent['dialogue_act']}"
            else:
                y = tag2id.get(sent['dialogue_act'])
                out += x + '\t' + str(y) + '\n'
    with open(path, 'w', encoding='utf-8') as f:
